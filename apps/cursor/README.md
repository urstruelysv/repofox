...
In Cursor, open **Extensions**, select **… → Install from VSIX…**, then choose
`apps/cursor/repofox-cursor-0.0.1.vsix`. Reload when Cursor asks.

## First-run setup

1. Open a disposable Git repository in Cursor and select the RepoFox icon in
   the Activity Bar.
...
Provider keys and GitHub tokens are stored in Cursor SecretStorage. They are
...
4. Open **Operations History** after completion to inspect the receipt. Close
   and reopen Cursor; the repository-scoped history remains available.
